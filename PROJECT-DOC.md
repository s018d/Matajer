...
| `/signup` GET/POST | تسجيل ذاتي: اسم متجر + مستخدم + كلمة مرور (8+ حرف مع رقم وحرف) + **كابتشا حسابية** + **حقل honeybot** + حد 5/IP/ساعة + دعم `?ref=slug` للإحالات + **تجربة برو 14 يوم تلقائياً** (المدة من trial_days) |
...
| `/panel/billing` + `/billing/request` | باقات: برو 10000/شهر، 25000/3 شهور، 100000/سنة — أعمال 25000/شهر، طلب ترقية مع **رقم التحويل + رفع صورة الإثبات** → payments reported |
...
- **التجربة:** أي متجر جديد يحصل **برو 14 يوم تلقائياً** (المدة قابلة للتغيير من /admin/site → trial_days)، وتظهر الباقي وحالة الانتهاء في أعلى اللوحة.
...
**إعدادات المنصة:** site_name=دُكّان Dukkan — واتساب 9647831020026 — تيليغرام/إنستغرام @s018d — تيك توك @s018a — مجاني=10 منتجات — أسعار 10000/25000/100000 — أعمال 25000 — trial_days=14 — pay_account فارغ (لم يُحدد بعد!).
...
| 2 | تجربة برو 14 يوم عند التسجيل (قابلة للتعديل) | routes/site.js, routes/admin.js, views/admin/site.ejs, views/signup.ejs, db.js |
