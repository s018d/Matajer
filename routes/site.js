...
router.get('/sitemap.xml', (req, res) => {
  ...
  let stores = [];
  try { stores = db.prepare("SELECT slug FROM stores WHERE status='active'").all(); } catch { stores = []; }
  const now = new Date().toISOString().slice(0,10);
  const staticUrls = ['', '/faq', '/privacy', '/terms'].map(p => `<url><loc>${host}${p}</loc><lastmod>${now}</lastmod><priority>0.8</priority></url>`).join('\n');
  const storeUrls = stores.map(s => `<url><loc>${host}/s/${s.slug}</loc><lastmod>${now}</lastmod><priority>0.9</priority></url>`).join('\n');
